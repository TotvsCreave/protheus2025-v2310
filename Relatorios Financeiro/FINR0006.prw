...
/*
|=============================================================================|
| PROGRAMA..: FINR0006   |  ANALISTA: Sidnei Lempk    |    DATA: 01/07/2019   |
|=============================================================================|
| DESCRICAO.: Rotina para impressão de vales por carga.		      			  |
|=============================================================================|
| PARÂMETROS:                                                                 |
...
user function FINR0006()
	...
	// TES de Bonificação
	Private cTesBon  := GETMV( "UV_TESBONI" ) //510|512|521|511|539|548
	...
	oPrn:=TMSPrinter():New("Emissão de vales por carga - FINR0006",.F.,.F.)

	oPrn:SetPortrait()
	oPrn:SetPaperSize(DMPAPER_A4)
	RptStatus({|| Imprime()},"Emissão de vales por carga - FINR0006")
...
Static Function Imprime()
	...
	cQry += "Case When D2_CF = '5910' then 'B' else 'V' end as Tipo_Pedido, D2_CF as CFOP, "
	...
	cQry += "Trim(C5_XHORIMP) as Hora_Imp,  "

	cQry += "Case when (select QTD_CTRLCX from weblog_ctrlcx_saldos Saldos "
	cQry += "Where Substr(CLIENTE_CTRLCX,1,1) not in ('F','P') and CLIENTE_CTRLCX = C9_CLIENTE and LOJACLIENTE_CTRLCX = C9_LOJA) is null then 0 "
	cQry += "else"
	cQry += "(select QTD_CTRLCX from weblog_ctrlcx_saldos Saldos "
	cQry += "Where Substr(CLIENTE_CTRLCX,1,1) not in ('F','P') and CLIENTE_CTRLCX = C9_CLIENTE and LOJACLIENTE_CTRLCX = C9_LOJA) End "
	cQry += "as Saldo_Cx, "
	cQry += "CxCaixas as CxItem "
	...
	Do While !TMP->(Eof())
		...
		*/
		...
		Do while TMP->Pedido = cPed_Ant
			...
			ElseIF TMP->Media = 'N'
				IF TMP->FatConv = 0
					cLinha := Alltrim(Transform(TMP->XQuant  ,"@E 99,999"))+" "
					oPrn:Say(nLin,0550,cLinha,oFont08,030,,0 )
...
Static Function CabVale()
	...
	oPrn:Say(nLin,0750,"PIX",oFont12b,030,,,, )
	oPrn:Say(nLin,1149,"|",oFont10,030,,,, )

	//2ª Via - Retornar assinada
	oPrn:SayBitmap(nLin,1170,cBitMap,123,70)
	oPrn:Say(nLin,1300,"Avecre Abatedouro Ltda",oFont12b,030,,,, )
	oPrn:Say(nLin,1850,"PIX",oFont12b,030,,,, )

	nLin += 60

	oPrn:Say(nLin,0150,dtoc(date())+" às "+time(),oFont10b,030,,,, )
	oPrn:Say(nLin,0750,"01464871000129",oFont12b,030,,,, )
	oPrn:Say(nLin,1149,"|",oFont10,030,,,, )
	oPrn:Say(nLin,1300,dtoc(date())+" às "+time(),oFont10b,030,,,, )
	oPrn:Say(nLin,1850,"01464871000129",oFont12b,030,,,, )
...
Static Function ImpRodape()
	...
	cMsg += Alltrim(Transform(nSal_Cx,"@E 999,999"))
	cMsg += Space(10) + "Recolhidas: _______________"
