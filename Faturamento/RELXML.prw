...
static function GeraRelXML(lEnd)
	...
	if lBusSX3
		...
		for nca := 1 to len(aConfig)
			...
			If !Empty(cDesc)
				...
				aConfig[nca,2]:= GetSX3Cache(aConfig[nca,4],"X3_TIPO")//aContent[4] //SX3->X3_TIPO
				aConfig[nca,3]:= GetSX3Cache(aConfig[nca,4],"X3_DECIMAL")//aContent[6] //SX3->X3_DECIMAL
				aConfig[nca,5]:= GetSX3Cache(aConfig[nca,4],"X3_TAMANHO")//aContent[5] //SX3->X3_TAMANHO
	...
	If !Empty(cPerg)
		...
		//oSX1:AddGroup(cPerg)
		//oSX1:SearchGroup()
		//aSx1 := oSX1:GetGroup(cPerg)
		cQryX1:="Select X1_GRUPO,X1_ORDEM||' '||X1_PERGUNT,X1_TIPO,X1_VAR01 "
		cQryX1+=" from SX1000 x1 "
		cQryX1+=" where x1.D_E_L_E_T_=' ' and X1_GRUPO='"+cPerg+"'"
		cQryX1+=" ORDER BY X1_ORDEM
		aSx1 := QryArray(cQryX1)
		nX:=1
//Alert(Len(aSx1))
		//DbSelectArea("SX1")
		//DbSetOrder(1)
		//If MsSeek(Padr(cPerg,Len(X1_GRUPO),""))
		While nX <= Len(aSx1)  //!(SX1->(Eof())) .And. SX1->X1_GRUPO == Padr(cPerg,Len(X1_GRUPO),"")
			FWrite(nHandle,'   <Row ss:AutoFitHeight="0" ss:Height="15">'+CRLF)
			FWrite(nHandle,'    <Cell><Data ss:Type="String">Pergunta '+aSx1[nX,1]+'</Data></Cell>'+CRLF)//FWrite(nHandle,'    <Cell><Data ss:Type="String">Pergunta '+SX1->X1_ORDEM+'</Data></Cell>'+CRLF)
			FWrite(nHandle,'    <Cell><Data ss:Type="String">'+fTxtXML(AllTrim(aSx1[nX,2]))+'</Data></Cell>'+CRLF)
			Do Case
			Case aSx1[nX,3]/*SX1->X1_TIPO*/ == "C"
				cTemp := fTxtXML(AllTrim(&(aSx1[nX,4]/*SX1->X1_VAR01*/)))
			Case aSx1[nX,3] == "D"
				cTemp := DTOC(&(aSx1[nX,4]/*SX1->X1_VAR01*/))
			Case aSx1[nX,3] == "N"
				//If aSx1[2][1]/* SX1->X1_GSC */ == "C"
				//	cTemp := fTxtXML(AllTrim(&("SX1->X1_DEF0"+AllTrim(Str(&(aSx1[2][4]/* SX1->X1_VAR01 */))))))
				//Else
				cTemp := AllTrim(cValtochar(&(aSx1[nX,4]/* SX1->X1_VAR01 */)))
				//EndIf
			End Case
