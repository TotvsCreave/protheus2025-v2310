...
User Function AtendPed()
                                                                   
//Private _dEmissao   := Ctod("13/04/2020") //dDataBase
...
//	DEFINE MSDIALOG oDlgInc TITLE "Atendimento de Pedidos" FROM 000, 000  TO 650, 1330 COLORS 0, 16777215 PIXEL
	DEFINE MSDIALOG oDlgInc TITLE "Atendimento de Pedidos" FROM 000, 000  TO 650, 1465 COLORS 0, 16777215 PIXEL
...
//	oGrpPedidos:nWidth := 1000
	...
	oGrpEstoque := TGROUP():Create(oDlgInc)
	oGrpEstoque:cName := "oGrpEstoque"
	oGrpEstoque:cCaption := "Estoque"
	oGrpEstoque:nLeft := 825
	oGrpEstoque:nTop := 1
//	oGrpEstoque:nWidth := 505
	oGrpEstoque:nWidth := 640
	...
	oGrpBotoes := TGROUP():Create(oDlgInc)
	oGrpBotoes:cName := "oGrpBotoes"
	oGrpBotoes:cCaption := ""
	oGrpBotoes:nLeft := 825
	oGrpBotoes:nTop := 576
	oGrpBotoes:nWidth := 640
	oGrpBotoes:nHeight := 70
	...
	@ 010,040 MSGET oEmissao VAR _dEmissao WHEN .F. SIZE 50,10 FONT oFont2 OF oDlgInc PIXEL  	 	

	@ 013,110 SAY "Filtro:"  SIZE 50,10 FONT oFont2 OF oDlgInc PIXEL                                                
	oCombo1 := TComboBox():New(013,130,{|u|if(PCount()>0,cCombo1:=u,cCombo1)},aItems,55,10,oDlgInc,,,,,,.T.,,,,,,,,,'cCombo1')
	@ 013,190 MSGET oFiltro VAR _cFiltro WHEN ValFil(oCombo1:nAt) SIZE 160,08 OF oDlgInc PIXEL  		
	@ 013,350 BUTTON oAplic PROMPT "Aplicar" SIZE 053, 010 OF oDlgInc PIXEL Action (ProcFilt(oCombo1:nAt,RTrim(_cFiltro)))
	
	@ 272,007 BUTTON oLegen1 PROMPT "Legenda" SIZE 400, 008 OF oDlgInc PIXEL Action (LEGEND())
	@ 272,417 BUTTON oLegen2 PROMPT "Legenda" SIZE 313, 008 OF oDlgInc PIXEL Action (LEGEND2())
	...
	@ 293,275 MSGET oTroca  VAR _cTroca WHEN .F. SIZE 20,10 FONT oFont2 OF oDlgInc PIXEL  
	...
	@ 310,120 SAY "Dias p/Entrega:" SIZE 100,10 FONT oFont2 OF oDlgInc PIXEL                                                
	@ 308,170 MSGET oEntrega VAR _cEntrega WHEN .F. SIZE 050,10 FONT oFont2 OF oDlgInc PIXEL  
	@ 308,275 BUTTON oSomator PROMPT "Somatório p/ Carga" SIZE 104, 010 FONT oFont2 OF oDlgInc PIXEL Action (ExibeCG())

	@ 299,520 BUTTON oSalvar PROMPT "Salvar"  SIZE 050, 015 OF oDlgInc PIXEL Action (Processa({|| GravaAlt() },"Salvando alterações..."),oDlgInc:End())
	@ 299,590 BUTTON oSair   PROMPT "Sair"    SIZE 050, 015 OF oDlgInc PIXEL Action (oDlgInc:End())     
...
// Gilbert - 13/05/2020 - Alteração na ordem das colunas do grid Estoques conforme solicitação do Felipe	
/*
...
// FIM - Gilbert - 13/05/2020
    ...
    oEst := MsNewGetDados():New( 010, 417, 270, 730, GD_UPDATE, "AllwaysTrue", "AllwaysTrue", "AllwaysTrue",aCpoEst, 0, 999, "AllwaysTrue", "", "AllwaysTrue", oDlgInc, aCabEst, aEst)
...
Static Function ProcFilt(nId,cFiltro)
	...
	ElseIf nId == 11 // Carga Totalmente Atendida
		aPedBrw := {}
		For x:=1 to Len(aPed)
...
Private nConvDesm := _nQtAtend/_nQtAteUN  // Variável utilizada no calculo do peso no desmembramento
	...
	// cArq2:=Criatrab(_aCampos,.T.)
	// dbUseArea(.t.,,carq2,"TEMP2")
	// Index On STATUS+PROD  To &cArq2
...
//		aAdd(aEst, { oBmpAux, TMP->B2_QATU, TMP->B2_QTSEGUM, TMP->B1_DESC, Alltrim(TMP->B2_COD), .F. })
	...
	cQuery := "SELECT SC9.C9_CARGA CARGA, SC9.C9_PEDIDO PEDIDO, SUBSTR(SA3.A3_NREDUZ,1,12) VEND, SC5.C5_CLIENTE CLIENTE, SC9.C9_PRODUTO PRODUTO, SC6.C6_DESCRI DESCRI, "
