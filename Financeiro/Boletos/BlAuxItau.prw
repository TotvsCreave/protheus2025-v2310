...
/*/
|=============================================================================|
| PROGRAMA..: BLITAU    |  ANALISTA: Fabiano Cintra   |    DATA: 30/07/2014   |
|=============================================================================|
| DESCRICAO.: Rotina para impressão de boleto de cobrança do Banco Itaú em    |
|             formato gráfico.                                                |
|=============================================================================|
| PARÂMETROS:                                                                 |
...
|=============================================================================|
/*/
User Function BL2ITAU(_cPrefixo, _cTitulo, _cAgencia, _cConta)

	//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
	//³ Define Variaveis Locais (Basicas)                            ³
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	Local cDesc1  := "Boleto Itaú"
	...
	"",;
	"PROTESTO NO 5° DIA ÚTIL APÓS O VENCIMENTO.",;
	"",;
	""} 
	Private CB_RN_NN     := {}
	Private nRec         := 0
	...
	cTitulo2  := _cTitulo     
	cPrefixo  := _cPrefixo                                                       
	cAgencia  := "6116"//_cAgencia
	cConta    := "02360"//_cConta
	cDigito   := "4"//mv_par06
	cCarteira := "109"//mv_par07	
...
Static Function MontaRel(lEnd,WnRel,cString)
	...
	cQuery += "       SE1.E1_LOJA, SE1.E1_DECRESC, SE1.E1_ACRESC, SE1.E1_VENCTO, SE1.E1_VALOR, SE1.E1_PORTADO, SE1.E1_NUMBCO "
	...
	cQuery += "      SE1.E1_NUM      >= '" + cTitulo1  + "' AND SE1.E1_NUM <= '" + cTitulo2  + "' "
	cQuery += "ORDER BY SE1.E1_PREFIXO, SE1.E1_NUM, SE1.E1_PARCELA"
	If Alias(Select("TRX")) = "TRX"
		TRX->(dBCloseArea())
	Endif
	...
	Do While !Eof()		
		...
		lContinua := .T.	
		If !Empty(TRX->E1_PORTADO) .and. TRX->E1_PORTADO <> "341" .and. !Empty(TRX->E1_NUMBCO)
			If  !MsgYesNo("Título "+TRX->E1_PREFIXO+TRX->E1_NUM+TRX->E1_PARCELA+" já possui boleto impresso para o banco "+TRX->E1_PORTADO+"." +chr(10)+chr(10)+;
			"Deseja reimprimir boleto e transferir título para o Banco Itaú? ")
				lContinua := .F.
			Endif
		Endif
		...
		If lContinua		
			...
			nParcela := At(TRX->E1_PARCELA,"ABCDEFGHIJKLMNOPQRSTUV")
			If nParcela = 0
				cParcela := ''
			ElseIF nParcela <= 9
			...
			_cNossoNum := StrZero(Val(Alltrim(TRX->E1_NUM)+cParcela),8)

			DbSelectArea("SA1")
			DbSetOrder(1)
			DbSeek(xFilial()+TRX->E1_CLIENTE+TRX->E1_LOJA,.T.)
			...
			dVencto  := CtoD(Substr(TRX->E1_VENCTO,7,2)+"/"+Substr(TRX->E1_VENCTO,5,2)+"/"+Substr(TRX->E1_VENCTO,1,4))
			dEmissao := CtoD(Substr(TRX->E1_EMISSAO,7,2)+"/"+Substr(TRX->E1_EMISSAO,5,2)+"/"+Substr(TRX->E1_EMISSAO,1,4))
			CB_RN_NN    := Ret_cBarra(Subs(aDadosBanco[1],1,3)+"9",aDadosBanco[3],aDadosBanco[4],aDadosBanco[5],AllTrim(_cNossoNum),(TRX->E1_SALDO-nVLNCC-TRX->E1_DECRESC+TRX->E1_ACRESC),dVencto)
			aDadosTit   := { AllTrim(TRX->E1_NUM)+AllTrim(TRX->E1_PARCELA)				,;  // [1]  Número do título
			dEmissao                                  				,;  // [2]  Data da emissão do título
			Date()                                  					,;  // [3]  Data da emissão do boleto
			dVencto                                  				,;  // [4]  Data do vencimento
			TRX->E1_SALDO                                     		,;  // [5]  Valor do título &&10/12/2009 Linha Original
			CB_RN_NN[3]                             					,;  // [6]  Nosso número (Ver fórmula para calculo)
			TRX->E1_PREFIXO                               			,;  // [7]  Prefixo da NF
			TRX->E1_TIPO	                           					,;  // [8]  Tipo do Titulo
			nVLNCC                                                    ,;	// [9]  Valor NCC 
			TRX->E1_DECRESC                                            }  // [10]	Valor Desconto
...
Static Function Impress(oPrint,aBitmap,aDadosEmp,aDadosTit,aDadosBanco,aDatSacado,aBolText,CB_RN_NN)
	...
	If File(aBmp)   // LOGOTIPO
		...
		oPrint:Say      ( 0070,0230,"Banco Itaú S.A.",oFont14n )
	Else
	...
	oPrint:Say  (0200+_nAux,1060,aDadosBanco[3]+"/"+aDadosBanco[4]+"-"+aDadosBanco[5],oFont10)
	...
	oPrint:Say  (0300+_nAux,100 ,aDatSacado[1]                                    ,oFont10)	//Nome
	...
	oPrint:Say  (0550+_nAux, 100,"Endereço do Beneficiário"                                           ,oFont8)
	oPrint:Say  (0600+_nAux, 150,AllTrim(aDadosEmp[2])+" - "+aDadosEmp[3]+" - "+aDadosEmp[4]       	,oFont10)
	oPrint:Line (0650, 100,0650,2300 )
	...
	If File(aBmp) // LOGOTIPO
		...
		oPrint:Say      ( 0865,0210,"Banco Itaú S.A.",oFont12n )	
	Else
		oPrint:Say  (0865,0100,"Banco Itaú S.A.",oFont14n )	
	EndIf
	oPrint:Say  (0860+_nAux,0570,"341-7",oFont16n )	
	oPrint:Say  (0865+_nAux,1800,"RECIBO DO PAGADOR",oFont14n)
	// Verticais
	...
	oPrint:Say  (0950+_nAux,400 ,"PAGÁVEL EM QUALQUER BANCO ATÉ O VENCIMENTO."    ,oFont9 )
	oPrint:Say  (0990+_nAux,400 ,"APÓS O VENCIMENTO, SOMENTE NO ITAÚ."            ,oFont9 )
	oPrint:Say  (0930+_nAux,1910,"Vencimento"                                     ,oFont8 )
	oPrint:Say  (0970+_nAux,2115,AllTrim(DTOC(aDadosTit[4]))                      ,oFont10) 
	//_________________________________________________________________________________________________________________________________________________
	oPrint:Line (1030,0100,1030,2300 )                                              
	oPrint:Say  (1030+_nAux,0100,"Beneficiário"                                           ,oFont8)
	oPrint:Say  (1060+_nAux,0100,aDadosEmp[1]                                 	   ,oFont9)           
	oPrint:Say  (1060+_nAux,1450,aDadosEmp[6]                                 	   ,oFont9)           
	oPrint:Say  (1030+_nAux,1910,"Agência/Cód.Beneficiário"                            ,oFont8)
	oPrint:Say  (1060+_nAux,2080,AllTrim(aDadosBanco[3]+"/"+aDadosBanco[4]+"-"+aDadosBanco[5]),oFont10)           
	//_________________________________________________________________________________________________________________________________________________
	...
	oPrint:Say  (1100+_nAux,1005,"Espécie Doc."                                     ,oFont8)
	oPrint:Say  (1130+_nAux,1050,"DMI"									          ,oFont9) //Tipo do Titulo
	oPrint:Say  (1100+_nAux,1355,"Aceite"                                           ,oFont8)
	oPrint:Say  (1130+_nAux,1455,"N"                                                ,oFont9)
	oPrint:Say  (1100+_nAux,1555,"Data do Processamento"                            ,oFont8)
	oPrint:Say  (1130+_nAux,1655,DTOC(aDadosTit[3])                                 ,oFont9) // Data impressao
	oPrint:Say  (1100+_nAux,1910,"Nosso Número"                                     ,oFont8)
	oPrint:Say  (1130+_nAux,2000,AllTrim(Substr(aDadosTit[6],1,3)+"/"+Substr(aDadosTit[6],4)),oFont10)                
	//_________________________________________________________________________________________________________________________________________________
	...
	oPrint:Say  (1170+_nAux,755 ,"Espécie"                                           ,oFont8)
	oPrint:Say  (1200+_nAux,805 ,"R$"                                                ,oFont9)
	oPrint:Say  (1170+_nAux,1005,"Quantidade"                                        ,oFont8)
	oPrint:Say  (1170+_nAux,1555,"Valor"                                             ,oFont8)
	oPrint:Say  (1170+_nAux,1910,"Valor do Documento"                          	   ,oFont8)
	oPrint:Say  (1200+_nAux,2100,Transform(aDadosTit[5],"@E 999,999,999.99")         ,oFont10)
	//_________________________________________________________________________________________________________________________________________________
	oPrint:Line (1240,100,1240,2300)
	oPrint:Say  (1240+_nAux,100 ,"Instruções (Todas informações deste bloqueto são de exclusiva responsabilidade do beneficiário)", oFont8 )
	oPrint:Say  (1300+_nAux,150 ,aBolText[1] + AllTrim(Transform((TRX->E1_SALDO*(_nTxper/100)),"@E 999,999.99")) + " AO DIA.", oFont10 )
	...
	oPrint:Say  (1500+_nAux,150 ,aBolText[5], oFont10 )
	oPrint:Say  (1240+_nAux,1910,"(-)Desconto/Abatimento", oFont8 )
	If aDadosTit[10] > 0
		oPrint:Say  (1270+_nAux,2100,Transform(aDadosTit[10],"@E 999,999,999.99"), oFont10 )                
	Endif
	//_________________________________________________________________________________________________________________________________________________
	oPrint:Line (1310,1900,1310,2300 )
	oPrint:Say  (1310+_nAux,1910,"(-)Outras Deduções"                                                                        ,oFont8 )
	//oPrint:Say  (1340,2130,Transform(aDadosTit[10],"@E 999,999,999.99"),oFont10) 
	//_________________________________________________________________________________________________________________________________________________
	oPrint:Line (1380,1900,1380,2300 )
	oPrint:Say  (1380+_nAux,1910,"(+)Mora/Multa"                                                                             ,oFont8 )
	//_________________________________________________________________________________________________________________________________________________
	...
	oPrint:Say  (1600+_nAux,0400,aDatSacado[1],oFont10)                          
	if Len(Alltrim(aDatSacado[7])) == 14
		oPrint:Say  (1600+_nAux,1700 ,"C.N.P.J.: "+TRANSFORM(aDatSacado[7],"@R 99.999.999/9999-99"),oFont10)
	else
		oPrint:Say  (1600+_nAux,1700 ,"C.P.F.: "+TRANSFORM(aDatSacado[7],"@R 999.999.999-99"),oFont10)
	endif
	oPrint:Say  (1640+_nAux,0400,aDatSacado[3]                                                                                     ,oFont10)
	oPrint:Say  (1680+_nAux,0400,Substr(aDatSacado[6],1,5)+"-"+Substr(aDatSacado[6],6,3)+"    "+aDatSacado[4]+"     "+aDatSacado[5],oFont10) // CEP+Cidade+Estado
	oPrint:Say  (1680+_nAux,2000,Substr(aDadosTit[6],1,3)+"/"+Substr(aDadosTit[6],4,8)+"-"+Substr(aDadosTit[6],13,1)               ,oFont10)

	oPrint:Say  (1690+_nAux,1600,"Código de Baixa",oFont8)
	oPrint:Say  (1690+_nAux,0100,"Sacador/Avalista",oFont8)
	oPrint:Line (1730,0100,1730,2300)

	oPrint:Say  (1730+_nAux, 100,"Endereço do Beneficiário"                                           ,oFont8)
	oPrint:Say  (1780+_nAux, 150,AllTrim(aDadosEmp[2])+" - "+aDadosEmp[3]+" - "+aDadosEmp[4]       	,oFont10)
	oPrint:Line (1830, 100,1830,2300 )

	//_________________________________________________________________________________________________________________________________________________
	oPrint:Say  (1830+_nAux,1910,"Autenticação Mecânica",oFont8)
	...
	If File(aBmp)  // LOGOTIPO
		...
		oPrint:Say      ( 2080,0210,"Banco Itaú S.A.",oFont12n )
	Else
		oPrint:Say  (2080,100,"Banco Itaú S.A.",oFont14n )	
	EndIf
	oPrint:Say  (2075+_nAux,0570,"341-7",oFont16n ) 
	oPrint:Say  (2080+_nAux,0800,CB_RN_NN[2],oFont14n)		//Linha Digitavel do Codigo de Barras
	// Verticais
	oPrint:Line (2145+_nAux,550,2075,550)                                                     
	oPrint:Line (2145+_nAux,730,2075,730)                                                                                                    
	//_________________________________________________________________________________________________________________________________________________
	oPrint:Line (2145,100,2145,2300)
	oPrint:Say  (2145+_nAux,100 ,"Local de Pagamento"                              ,oFont8 )
	oPrint:Say  (2165+_nAux,400 ,"ATÉ O VENCIMENTO, PREFERENCIALMENTE NO ITAÚ."    ,oFont9 )
	oPrint:Say  (2205+_nAux,400 ,"APÓS O VENCIMENTO, SOMENTE NO ITAÚ.         "    ,oFont9 )
	oPrint:Say  (2145+_nAux,1910,"Vencimento"                                      ,oFont8 )
	oPrint:Say  (2185+_nAux,2115,DTOC(aDadosTit[4])                                ,oFont10)
	//_________________________________________________________________________________________________________________________________________________
	oPrint:Line (2245,100,2245,2300)
	oPrint:Say  (2245+_nAux,100 ,"Beneficiário"                                        ,oFont8)
	oPrint:Say  (2275+_nAux,100 ,aDadosEmp[1]                                 	,oFont9)              
	oPrint:Say  (2275+_nAux,1450,aDadosEmp[6]                                 	,oFont9)           
	oPrint:Say  (2245+_nAux,1910,"Agência/Cód.Beneficiário"                         ,oFont8)
	oPrint:Say  (2275+_nAux,2080,aDadosBanco[3]+"/"+aDadosBanco[4]+"-"+aDadosBanco[5],oFont10)
	//_________________________________________________________________________________________________________________________________________________
	...
	oPrint:Say  (2315+_nAux,1005,"Espécie Doc."                                     ,oFont8)
	oPrint:Say  (2345+_nAux,1050,"DMI"  										      ,oFont9) //Tipo do Titulo
	oPrint:Say  (2315+_nAux,1355,"Aceite"                                           ,oFont8)
	oPrint:Say  (2345+_nAux,1455,"N"                                                ,oFont9)
	oPrint:Say  (2315+_nAux,1555,"Data do Processamento"                            ,oFont8)
	oPrint:Say  (2345+_nAux,1655,DTOC(aDadosTit[3])                                 ,oFont9) // Data impressao
	oPrint:Say  (2315+_nAux,1910,"Nosso Número"                                     ,oFont8)       
	oPrint:Say  (2345+_nAux,2000,Substr(aDadosTit[6],1,3)+"/"+Substr(aDadosTit[6],4),oFont10)  
	//_________________________________________________________________________________________________________________________________________________
	...
	oPrint:Say  (2385+_nAux,755 ,"Espécie"                                           ,oFont8)      
	oPrint:Say  (2415+_nAux,805 ,"R$"                                                ,oFont9)      
	oPrint:Say  (2385+_nAux,1005,"Quantidade"                                        ,oFont8)      
	oPrint:Say  (2385+_nAux,1555,"Valor"                                             ,oFont8)      
	oPrint:Say  (2385+_nAux,1910,"Valor do Documento"                          	   ,oFont8)      
	oPrint:Say  (2415+_nAux,2100,Transform(aDadosTit[5],"@E 999,999,999.99")         ,oFont10)  
	//_________________________________________________________________________________________________________________________________________________
	oPrint:Line (2455,100,2455,2300 )
	oPrint:Say  (2455+_nAux,100 ,"Instruções (Todas informações deste bloqueto são de exclusiva responsabilidade do beneficiário)",oFont8 ) 
	oPrint:Say  (2515+_nAux,150 ,aBolText[1] + AllTrim(Transform((TRX->E1_SALDO*(_nTxper/100)),"@E 999,999.99")) + " AO DIA.",oFont10)
	...
	oPrint:Say  (2715+_nAux,150 ,aBolText[5], oFont10)
	oPrint:Say  (2455+_nAux,1910,"(-)Desconto/Abatimento", oFont8 )
	If aDadosTit[10] > 0
		oPrint:Say  (2485+_nAux,2100,Transform(aDadosTit[10],"@E 999,999,999.99"), oFont10)
	Endif
	//_________________________________________________________________________________________________________________________________________________
	oPrint:Line (2525,1900,2525,2300 )                                                            
	oPrint:Say  (2525+_nAux,1910,"(-)Outras Deduções"                             ,oFont8)            
	//oPrint:Say  (2555,2130,Transform(aDadosTit[10],"@E 999,999,999.99")     ,oFont10) 
	//_________________________________________________________________________________________________________________________________________________
	oPrint:Line (2595,1900,2595,2300 )
	oPrint:Say  (2595+_nAux,1910,"(+)Mora/Multa"                                  ,oFont8)
	//_________________________________________________________________________________________________________________________________________________
	...
	oPrint:Say  (2815+_nAux,400 ,aDatSacado[1]             ,oFont10)
	IF LEN(Alltrim(aDatSacado[7])) == 14
		oPrint:Say  (2815+_nAux,1700 ,"C.N.P.J.: "+TRANSFORM(aDatSacado[7],"@R 99.999.999/9999-99"),oFont10)
	ELSE
		oPrint:Say  (2815+_nAux,1700 ,"C.P.F.: "+TRANSFORM(aDatSacado[7],"@R 999.999.999-99"),oFont10)
	ENDIF
	oPrint:Say  (2855+_nAux,400 ,aDatSacado[3]                                    ,oFont10)      
	oPrint:Say  (2895+_nAux,400 ,aDatSacado[6]+"    "+aDatSacado[4]+" - "+aDatSacado[5],oFont10) // CEP+Cidade+Estado  
	oPrint:Say  (2895+_nAux,2000,Substr(aDadosTit[6],1,3)+"/"+Substr(aDadosTit[6],4,8)+"-"+Substr(aDadosTit[6],13,1)  ,oFont10)      

	oPrint:Say  (2905+_nAux,1600,"Código de Baixa",oFont8)
	...
	DbSelectArea("SE1")
	DbSetOrder(1)
	DbSeek(xFilial()+TRX->E1_PREFIXO+TRX->E1_NUM+TRX->E1_PARCELA+TRX->E1_TIPO,.T.)
	RecLock("SE1",.f.)                                           
	//SE1->E1_PORTADO := aDadosBanco[1]
	//SE1->E1_AGEDEP  := aDadosBanco[3]
	//SE1->E1_CONTA   := aDadosBanco[4]+aDadosBanco[5]
	//SE1->E1_SITUACA := "1"
	SE1->E1_NUMBCO  := Substr(aDadosTit[6],4,8)+Substr(aDadosTit[6],13,1)   // Nosso número
	MsUnlock()
...
//	CB_RN_NN    := Ret_cBarra(Subs(aDadosBanco[1],1,3)+"9",aDadosBanco[3],aDadosBanco[4],aDadosBanco[5],"175"+AllTrim(E1_NUM),(E1_SALDO-_nVlrAbat) )
//
/*/
|=====================================================================|
...
Static Function Ret_cBarra(cBanco,cAgencia,cConta,cDacCC,cNroDoc,nValor,dVencto)
	...
	dvnn := modulo10(s) // digito verificador Agencia + Conta + Carteira + Nosso Num
