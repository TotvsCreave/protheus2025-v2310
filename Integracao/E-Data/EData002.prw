...
Static function fBuscaCanditados()
	...
	cTstQry := "Select * from DAK000 DAK "
    cTstQry += "Where DAK.D_E_L_E_T_ <> '*' "
	cTstQry += "and DAK_DATA  between '" + dtos(MV_PAR01) + "' and '" + dtos(MV_PAR02) + "' "
	cTstQry += "and DAK_COD  between '" + MV_PAR03 + "' and '" + MV_PAR04 + "' "

	If Alias(Select("cTstQry")) = "cTstQry"
		cTstQry->(dBCloseArea())
	Endif

	TCQUERY cTstQry NEW ALIAS "cTstQry"

	DBSelectArea("cTstQry")
	cTstQry->(DBGoTop())

	If cTstQry->(eof())
		Alert("Nenhum registro encontrado para a consulta. Verifique se todas as cargas tem veículo associado.", "Aviso")
        aRecnos := {}
		Return()
	Else
		Do While !cTstQry->(Eof())
			// Verifica se o veículo está vazio
			If Empty(cTstQry->DAK_CAMINH)
				Alert("Carga: " + cTstQry->DAK_COD + " não possui veículo associado.", "Aviso")
                aRecnos := {}
                Return()
			EndIf
			cTstQry->(DBSkip())
