...
±±³ 05/11/15 ³ Adriano       ³ Customizações específicas Avecre           ³±±
±±³ 03/12/15 ³ Gustavo       ³ Costura para o novo fonte com NT003        ³±±
±±³ 19/06/18 ³ Bruno Coelho  ³(APIA)Implementação de nova versão NFE4.0   ³±±
±±³ 09/04/19 ³ Gustavo       ³ Costura para o novo fonte com NT2018.005   ³±±
±±³ 12/04/19 ³ Gustavo       ³ Costura para fonte com benific/desoneração ³±±
±±³ 21/08/19 ³ Adriano       ³ Atualização ref. NT 2019-001               ³±±
±±³ 29/08/19 ³ Adriano       ³ Atualização ref. NT 2019-001               ³±±
±±³ 16/08/21 ³ Gilbert       ³ Atualização ref. NT 2020-006               ³±±
±±³ 24/02/22 ³ Celso         ³ Atualização release 33 do Protheus         ³±±
±±³ 20/05/24 ³ Gustavo       ³ Atualização release 2310 do Protheus       ³±±
±±³ 18/06/25 ³ Gustavo       ³ Atualização release 2410 do Protheus       ³±±
			...
			Else
				...
				Else
					BeginSql Alias cAliasSD2
						...
						D2_QUANT,D2_TOTAL,D2_DESCON,D2_VALFRE,D2_SEGURO,D2_PEDIDO,D2_ITEMPV,D2_DESPESA,D2_VALBRUT,D2_VALISS,D2_PRUNIT,
						D2_CLASFIS,D2_PRCVEN,D2_IDENTB6,D2_CODISS,D2_DESCZFR,D2_PREEMB,D2_DESCZFC,D2_DESCZFP,D2_LOTECTL,D2_NUMLOTE,D2_ICMSRET,D2_VALPS3,
							D2_ORIGLAN,D2_VALCF3,D2_VALIPI,D2_VALACRS,D2_PICM,D2_PDV,D2_CSOSN,D2_VALICM,D2_EMISSAO %Exp:cField% 
						FROM %Table:SD2% SD2
						WHERE
						SD2.D2_FILIAL  = %xFilial:SD2% AND
						SD2.D2_SERIE   = %Exp:SF2->F2_SERIE% AND
				...
				While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
					...
					If lContinua
						...
						If lConfTrib .AND. oICMSCfg <> NIL
							...
							lICMSDif	:= oICMSCfg['regras_escrituracao']['perc_diferimento'] > 0
							lDifNFDup	:= lICMSDif .AND. oICMSCfg['regras_escrituracao']['acao_total_nf'] $ '3'//6 – Diferido(Deduz NF e Duplicata)
							lDifExp		:= 66.66 == oICMSCfg['regras_escrituracao']['perc_diferimento']
						Else
						...
						If lConfTrib .AND. oPISCfg <> NIL
							lAgregPis	:= ValType(oPISCfg['regras_escrituracao']) == "J" .AND. oPISCfg['regras_escrituracao']['acao_total_nf'] $ '5/6'
						...
						If lConfTrib .AND. oCOFCfg <> NIL
							lAgregCOF	:= ValType(oCOFCfg['regras_escrituracao']) == "J" .AND. oCOFCfg['regras_escrituracao']['acao_total_nf'] $ '5/6'
			    ...
			    If nValTFecp > 0
					...
					EndIf
			    EndIf
                // Incluido por Celso em 24/02/2022 - Atualização para a release 33 do Protheus
			    // Tratamento de mensagens com base no NCM dos produtos - Adriano 05/11/2015
				cMsgTemp := ""
				if aDest[09] = "RJ"
					// Vendas para dentro do estado
					if aScan(aProd, {|x| left(x[5],4) == "0207"}) <> 0 .or. aScan(aProd, {|x| left(x[5],4) == "1601"}) <> 0
						cMsgTemp += " * REDUÇÃO DA BASE DE CÁLCULO DO ICMS CONFORME INCISO III DO ARTIGO 1º DA LEI 8792 DE 13/04/2020."
						cMsgTemp += " * ALÍQUOTA ZERO DE PIS E COFINS CONF LEI 10/925/2004 ART 1º INSCISO XIX E PARAGRAFO 4º."
					endif
					if	aScan(aProd, {|x| left(x[5],8) == "23011010"}) <> 0 .or.;
						aScan(aProd, {|x| left(x[5],8) == "15060000"}) <> 0 .or.;
						aScan(aProd, {|x| left(x[5],8) == "05059000"}) <> 0
						cMsgTemp += " * PRODUTOS ISENTOS DO ICMS, DE ACORDO COM A RESOLUÇÃO SEF 2884/1997."
					endif
				else
					// Vendas para fora do estado
					if aScan(aProd, {|x| left(x[5],4) == "0207"}) <> 0
						cMsgTemp += " * PRODUTOS COM A ALÍQUOTA 0 DE PIS E COFINS, DE ACORDO COM A LEI 10.925/2004 (ALTERADA PELA LEI 12.839/2013)."
					endif
					if	aScan(aProd, {|x| left(x[5],8) == "23011010"}) <> 0 .or.;
						aScan(aProd, {|x| left(x[5],8) == "15060000"}) <> 0 .or.;
						aScan(aProd, {|x| left(x[5],8) == "05059000"}) <> 0
						cMsgTemp += " * PRODUTOS COM A BASE DE CALCULO DO ICMS REDUZIDA EM 60%, DE ACORDO COM O CONVENIO ICMS 100/97."
					endif
				endif
				cMensCli += cMsgTemp
