...
// Alterado por Celso em 25/02/2022, para a release 33 do Protheus
...
// Final da alteração - Celso em 25/02/2022
...
±±³ 24/02/22 ³ Celso         ³(APIA)Implementação para release 33         ³±±
±±³ 20/05/24 ³ Gustavo       ³ Atualização release 2310 do Protheus       ³±±
±±³ 20/06/25 ³ Gustavo       ³ Atualização release 2410 do Protheus       ³±±
...
//Sidnei - CABECALHO DA NOTA - Manter pra proximas versões

		If SF2->F2_TIPO = "D" .and. SD2->D2_GRUPO >= '9000'
			cCliente := SF2->F2_CLIENTE + '-' + SF2->F2_LOJA + '  '+ Alltrim(Posicione("SA2",1,xFilial("SA2")+SF2->F2_CLIENTE + SF2->F2_LOJA,"A2_NOME")) + '/' + Alltrim(Posicione("SA2",1,xFilial("SA2")+SF2->F2_CLIENTE + SF2->F2_LOJA,"A2_NREDUZ"))
		Else
			cCliente := SF2->F2_CLIENTE + '-' + SF2->F2_LOJA + '  '+ Alltrim(Posicione("SA1",1,xFilial("SA1")+SF2->F2_CLIENTE + SF2->F2_LOJA,"A1_NOME")) + '/' + Alltrim(Posicione("SA1",1,xFilial("SA1")+SF2->F2_CLIENTE + SF2->F2_LOJA,"A1_NREDUZ"))
		Endif

		cEmisPed := 'Emissão: ' + Dtoc(Posicione("SC5",3,xFilial("SC5")+SF2->F2_CLIENTE + SF2->F2_LOJA + SD2->D2_PEDIDO,"C5_EMISSAO")) + ' Pedido: ' + SD2->D2_PEDIDO + ' Carga: ' + SF2->F2_CARGA
		cEmisPed += ' - Valor: ' + TRANSFORM(SF2->F2_VALBRUT,"@E 9,999,999.99") + ' Pedido do cliente: ' + Posicione("SC5",3,xFilial("SC5")+SF2->F2_CLIENTE + SF2->F2_LOJA + SD2->D2_PEDIDO,"C5_XPEDWMW")

		oDanfe:Say(025, 102, cCliente, oFont08:oFont)
		oDanfe:Say(035, 102, cEmisPed, oFont08:oFont)
...
if !lUsacolab
	...
	if odoc:consultar()
		...
		EndIf
        //Revisado por Celso em 24/02/2022 - revisão para implementação da release 33 
		//Altero o cRetorno para o XML padrão que foi enviado.
		//cRetorno := oDoc:cXml //Bruno Coelho 19-JUN-2018 - removida pela TOTVS na v12.1.17 NF4.0
...
//Acrescentado Sidnei a pedido Regis em 27/04/2020
/*
dbSelectArea("SD2")
dbSetOrder(3)
MsSeek(xFilial("SD2")+SF2->F2_DOC+SF2->F2_SERIE+SF2->F2_CLIENTE+SF2->F2_LOJA)
...
xMsg := "Dest.: " + Alltrim(NoChar(oDestino:_XNome:TEXT,lConverte)) + ' - Carga: ' + Posicione("SF2",1,xFilial("SF2")+StrZero(Val(oIdent:_NNf:Text),9)+SubStr(oIdent:_Serie:Text,1,3),"F2_CARGA")
oDanfe:Say(024, 102,xMsg,oFont07:oFont)

xMsg := "Emissão: " + Iif(oNF:_INFNFE:_VERSAO:TEXT >= "3.10",ConvDate(oIdent:_DHEmi:TEXT),ConvDate(oIdent:_DEmi:TEXT))  + " - Valor R$" + Transform(Val(oTotal:_ICMSTOT:_vNF:TEXT),"@e 999,999.99")
oDanfe:Say(029, 102,xMsg,oFont07:oFont)
*/
//Fim
