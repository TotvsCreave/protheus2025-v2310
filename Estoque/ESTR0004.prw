...
user function ESTR0004()
    ...
    cQry += "Select ZZ_DATA as DataOp, "
    cQry += "ZZ_GRUPO as Grupo, Trim(ZZ_DESCRI) as Desc_Grupo, ZZ_QUANT as Qtd, ZZ_PESO as Peso_Bruto, ZZ_PESOREA as Peso_Liq, "
    cQry += "ZZ_QTDCXG as Cx_Grd, ZZ_QTDCXP as Cx_Peq, ZZ_PRODDES as Cod_Prod, Trim(SB1.B1_DESC) as Desc_Prod, ZZ_OP as OP, ZZ_PROC as Proc, "
    cQry += "SZZ.R_E_C_N_O_ as Registro, ZZ_HORA as Hora "
    cQry += "from SZZ990 SZZ "
    cQry += "Inner Join SB1000 SB1 on B1_COD = ZZ_PRODDES and SB1.D_E_L_E_T_ <> '*' "
    
    If nForRel = 3
        cQry += "Where ZZ_DATA Between '" + dDtDe + "' and '" + dDtAte + "' and SZZ.D_E_L_E_T_ <> '*'  "
    Else
        cQry += "Where ZZ_DATA = To_Char(Sysdate,'YYYYMMDD') and SZZ.D_E_L_E_T_ <> '*' "
    Endif

    cQry += "Order By ZZ_DATA, ZZ_OP, ZZ_GRUPO, SZZ.R_E_C_N_O_, ZZ_HORA, ZZ_PRODDES "
