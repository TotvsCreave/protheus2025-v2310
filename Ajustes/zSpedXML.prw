...
User Function zSpedXML(cDocumento, cSerie, cIdentNfe, lMostra, cPasta)
	Local aArea        := GetArea()
	...
	cArqXML     := 'T:\Protheus_Data\Exp_NfeXml\'+Alltrim(cIdentNfe)+".xml"
	...
	If !Empty(cDocumento)
		...
		If oWebServ:RetornaNotas()
			...
			Else
				//("zSpedXML > Verificar parâmetros, documento e série não encontrados ("+cDocumento+"/"+cSerie+")...")
		...
		Else
			//("zSpedXML > "+IIf(Empty(GetWscError(3)), GetWscError(1), GetWscError(3))+"...")
