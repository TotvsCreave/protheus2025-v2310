...
User Function OMSE0001()

	Local nPesoRet := 0

	nPesoRet := u_zLeBalanca("DIGTRON")
...
User Function zLeBalanca(cMarca)
	...
	If ! Empty(cMarca)
		...
		Else
		Alert('Qualquer balança que utilize porta serial')
			cVelocid  := SuperGetMV("MV_X_VELOC", .F.,"9600")    //Velocidade
			cParidade := SuperGetMV("MV_X_PARID", .F.,"n")       //Paridade
			cBits     := SuperGetMV("MV_X_BITS",  .F.,"8")       //Bits
			cStopBits := SuperGetMV("MV_X_SBITS", .F.,"1")       //Stop Bit
			cFluxo    := SuperGetMV("MV_X_FLUXO", .F.,"")        //Controle de Fluxo
			nTempo    := SuperGetMV("MV_X_TEMPO", .F.,5)         //Tempo
			Alert('Configuração: ' + cPorta+":"+cVelocid+","+cParidade+","+cBits+","+cStopBits)
		EndIf
...
// Gilbert - 25/03/2021
		...
		//Se outras marcas
		Else
			//Montando a configuraÃ§Ã£o (Porta:Velocidade,Paridade,Bits,Stop)
			cConfig := cPorta+":"+cVelocid+","+cParidade+","+cBits+","+cStopBits

			//Guarda resultado se houve abertura da porta
			lRet := MSOpenPort(@nH,cConfig)

			//Se nÃ£o conseguir abrir a porta, mostra mensagem e finaliza
			If !lRet
				MsgStop("<b>Falha</b> ao conectar com a porta serial. Detalhes:"+;
				"<br><b>Porta:</b> "        +cPorta+;
				"<br><b>Velocidade:</b> "    +cVelocid+;
				"<br><b>Paridade:</b> "        +cParidade+;
				"<br><b>Bits:</b> "            +cBits+;
				"<br><b>Stop Bits:</b> "    +cStopBits,"AtenÃ§Ã£o")

			Else
				//Realiza a leitura
				For nX := 1 To 50
					//Obtendo o tempo de espera antes de iniciar a leitura da balanÃ§a    
					Sleep(nTempo)
					MSRead(nH,@cBuffer)

					//Se a linha retornada for igual ao tamanho limite, encerra o laÃ§o
					If Len(AllTrim(cBuffer)) == MAX_BUFFER
						Exit
					EndIf
				Next nX    

				//Verifica onde comeÃ§a o "E" e diminui 1 caracter
				nPosFim := At("E", cBuffer) - 1

				//Obtendo apenas o peso da balanÃ§a
				cPesoLido := StrTran(AllTrim(SubStr(cBuffer,2,nPosFim)),".","")
			EndIf

			//Encerra a conexÃ£o com a porta
			MSClosePort(nH,cConfig)


// FIM - Gilbert - 25/03/2021
